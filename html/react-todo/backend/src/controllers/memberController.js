// backend/src/controllers/memberController.js (VERSÃO FINAL E COMPLETA)
...
exports.createMember = async (req, res) => {
  ...
  try {
    if (!name || name.trim().length < 3) {
      return res.status(400).json({ message: 'O nome do membro é obrigatório e deve ter pelo menos 3 caracteres.' });
    }
    if (email && email.trim() !== '') {
      const existingMemberByEmail = await Member.findOne({ email: email.trim(), createdByUserId });
    ...
    res.status(201).json({ member: memberWithTaskCount, message: 'Membro criado com sucesso!' });
  } catch (error) {
...
exports.getUserMembers = async (req, res) => {
  try {
    const membersWithTaskCount = await Member.aggregate([
      ...
      { $lookup: { from: 'tasks', localField: '_id', foreignField: 'assignedMemberIds', as: 'assignedTasks' } },
      {
        $addFields: {
          completedTasks: {
            $size: {
      ...
      { $project: { name: 1, email: 1, role: 1, createdAt: 1, completedTasks: 1 } },
      { $sort: { name: 1 } }
    ]);
    res.status(200).json({ members: membersWithTaskCount, totalMembers: membersWithTaskCount.length });
  } catch (error) {
...
exports.getMemberById = async (req, res) => {
  try {
    ...
    const member = await Member.findOne({ _id: req.params.id, createdByUserId: req.user.id });
    if (!member) {
...
exports.updateMember = async (req, res) => {
  ...
  try {
    ...
    if (name !== undefined && (!name || name.trim().length < 3)) {
      return res.status(400).json({ message: 'O nome do membro deve ter pelo menos 3 caracteres.' });
    }
    const member = await Member.findOne({ _id: req.params.id, createdByUserId: req.user.id });
    if (!member) {
      return res.status(404).json({ message: 'Membro não encontrado.' });
    ...
    if (email && email.trim() !== '' && email !== member.email) {
      const existingMemberByEmail = await Member.findOne({ email: email.trim(), createdByUserId: req.user.id, _id: { $ne: req.params.id } });
      if (existingMemberByEmail) {
