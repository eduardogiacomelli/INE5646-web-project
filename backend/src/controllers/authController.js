...
// Removido o require('dotenv').config() daqui, pois será centralizado no server.js ou config/index.js
...
exports.loginUser = async (req, res) => {
  ...
  try {
    ...
    // Encontrar usuário pelo email. O Mongoose não retorna a senha por padrão se 'select: false' estiver no schema.
    // Se você ativou 'select: false' no modelo User.js para o campo password, use .select('+password')
    const user = await User.findOne({ email })//.select('+password'); // Descomente .select('+password') se necessário
